...
interface DashboardSidebarProps {
  ...
  isResizing?: boolean
  tickers?: string[]
  stats?: { totalFilings: number; uniqueTickers: number }
  ragasHistory?: any[]
}
...
export function DashboardSidebar({
  ...
  stats = { totalFilings: 210, uniqueTickers: 14 },
  ragasHistory = DEFAULT_RAGAS_HISTORY
  ...
  const displayTickers = tickers.length > 0 ? tickers : MOCK_TICKERS
  const displayStats = stats && stats.totalFilings > 0 ? stats : { totalFilings: 210, uniqueTickers: displayTickers.length }
  const displayRagasHistory = ragasHistory.length > 0 ? ragasHistory : DEFAULT_RAGAS_HISTORY

  // Get the latest RAGAS scores to display in the metrics cards
  const latestRagas = displayRagasHistory[displayRagasHistory.length - 1] || {
    faithfulness: 0.92,
    relevance: 0.94,
    precision: 0.89,
    recall: 0.91
  ...
  const toggleAllTickers = () => {
    if (selectedTickers.length === displayTickers.length) {
      onTickerChange([])
    } else {
      onTickerChange([...displayTickers])
    }
  }
    ...
    return (
      ...
      >
        <div className="flex flex-col items-center gap-6 w-full px-2" onClick={(e) => e.stopPropagation()}>
          <Link href="/" className="w-10 h-10 rounded-xl bg-gradient-to-tr from-slate-700 to-stone-800 flex items-center justify-center text-white font-bold text-lg shadow-md shadow-slate-500/10 hover:opacity-85 transition-opacity">
            S
          </Link>
      ...
      <div className="p-5 border-b border-stone-100">
        ...
        <div className="grid grid-cols-2 gap-3 mt-5">
          <div className="bg-stone-50 p-3 rounded-2xl border border-stone-100 flex flex-col">
            ...
            <span className="text-xl font-bold text-stone-700 mt-0.5">{displayStats.totalFilings}</span>
          ...
          <div className="bg-stone-50 p-3 rounded-2xl border border-stone-100 flex flex-col">
            ...
            <span className="text-xl font-bold text-stone-700 mt-0.5">{displayStats.uniqueTickers}</span>
            <span className="text-[9px] text-stone-400 mt-1 flex items-center gap-1">
        ...
        {activeTab === "filters" ? (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              >
                {selectedTickers.length === displayTickers.length ? "Deselect All" : "Select All"}
              </button>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-stone-50/80 p-3 rounded-2xl border border-stone-100">
                ...
                <span className="text-lg font-bold text-stone-700 mt-0.5">{Number(latestRagas.faithfulness).toFixed(2)}</span>
              ...
              <div className="bg-stone-50/80 p-3 rounded-2xl border border-stone-100">
                ...
                <span className="text-lg font-bold text-stone-700 mt-0.5">{Number(latestRagas.relevance || latestRagas.answer_relevance || 0.94).toFixed(2)}</span>
                <span className="text-[9px] text-stone-500 font-semibold block mt-1">Target: &gt;0.85</span>
              </div>
              <div className="bg-stone-50/80 p-3 rounded-2xl border border-stone-100">
                <span className="text-[10px] text-stone-400 font-semibold block">Context Prec.</span>
                <span className="text-lg font-bold text-stone-700 mt-0.5">{Number(latestRagas.precision || latestRagas.context_precision || 0.89).toFixed(2)}</span>
              ...
              <div className="bg-stone-50/80 p-3 rounded-2xl border border-stone-100">
                ...
                <span className="text-lg font-bold text-stone-700 mt-0.5">{Number(latestRagas.recall || latestRagas.context_recall || 0.91).toFixed(2)}</span>
                <span className="text-[9px] text-stone-500 font-semibold block mt-1">Target: &gt;0.80</span>
              </div>
            </div>
              ...
              <div className="h-44 w-full">
                {mounted && (
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={displayRagasHistory} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
