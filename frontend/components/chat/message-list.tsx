    ...
    >
      ...
      {messages.length === 0 && !error && !isStreaming && (
        <div className="flex flex-col items-center justify-center h-full text-center text-stone-400">
          ...
          <p className={`text-lg font-bold text-stone-700 ${hasAnimated ? "text-blur-intro" : ""}`}>
            SEC Filing Research Assistant
          </p>
          <p className={`text-sm mt-1 text-stone-500 max-w-md ${hasAnimated ? "text-blur-intro-delay" : ""}`}>
            Ask questions about 10-K & 10-Q reports, run multi-hop comparisons, or analyze corporate risks.
          </p>
        </div>
      )}
